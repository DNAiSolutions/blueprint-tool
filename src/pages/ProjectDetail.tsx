...
import {
  ...
  RefreshCw, MessageSquare, ArrowRight,
} from 'lucide-react';
...
/* ================================================================
   MOCK DATA — shown when Supabase tables are empty
   ================================================================ */
const MOCK_PROJECT = {
  id: 'mock-1',
  name: 'DigitalDNA',
...
const MOCK_CONTENT = [
  ...
  { id: '2', title: 'Tue — "Your neighbors hired us. Here\'s what happened."', status: 'pending', revision: 0, date: 'Apr 8' },
  ...
  { id: '5', title: 'Fri — "Why we turn down 30% of leads (and you should too)"', status: 'approved', revision: 0, date: 'Apr 11' },
];
...
export default function ProjectDetail() {
  ...
  const displayName = hasRealData ? (clientName ?? project.name) : MOCK_PROJECT.name;
  const displayStatus = hasRealData ? project.status : MOCK_PROJECT.status;
  const displayServices = hasRealData ? serviceTypes : MOCK_PROJECT.services;

  // Determine available tabs based on services
  ...
  return (
    <AppLayout>
      ...
      <div className="flex-1 overflow-auto p-6 scrollbar-thin">
        ...
        {activeTab === 'automations' && <AutomationsTab projectId={id} hasRealData={hasRealData} />}
      ...
      <div className="rounded-lg bg-card border border-border p-5">
        ...
        <div className="space-y-2">
          {[
            ...
            { text: 'Support ticket #2 picked up by support agent', time: '1 day ago', icon: LifeBuoy },
...
function ContentTab({ projectId, clientId, hasRealData }: { projectId?: string; clientId?: string; hasRealData: boolean }) {
  ...
  const displayContent = useReal
    ? clientApprovals.map(a => ({
        id: a.id,
        title: `Content piece #${a.revision_number > 0 ? `(Rev ${a.revision_number})` : '(New)'}`,
        status: a.status,
...
/* ================================================================
   AUTOMATIONS TAB — GHL subaccount pipelines + automation trees
   ================================================================ */
function AutomationsTab({ projectId, hasRealData }: { projectId?: string; hasRealData: boolean }) {
  return (
    <div className="space-y-6">
      {/* GHL Pipelines */}
      <div className="rounded-lg bg-card border border-border p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold">GHL Pipelines</h3>
          <Button variant="outline" size="sm" className="gap-1.5 text-xs">
            <RefreshCw className="h-3 w-3" /> Sync from GHL
          </Button>
        </div>
        <div className="space-y-4">
          {MOCK_GHL_PIPELINES.map(pipeline => (
            <div key={pipeline.name}>
              ...
              <div className="flex gap-2">
                {pipeline.stages.map(stage => (
                  <div key={stage.name} className="flex-1 p-3 rounded-lg bg-background border border-border/50 text-center">
                    <p className="text-lg font-bold text-accent">{stage.count}</p>
