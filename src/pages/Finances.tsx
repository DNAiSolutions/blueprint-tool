...
const mockTransactions = [
  { id: 'tr1', date: 'Apr 5', desc: 'Acme Pressure Washing — Monthly', amount: 1356, category: 'Revenue', type: 'income', client: 'Acme Pressure Washing' },
  { id: 'tr2', date: 'Apr 5', desc: 'Delta Pool Services — Monthly', amount: 1200, category: 'Revenue', type: 'income', client: 'Delta Pool Services' },
  { id: 'tr3', date: 'Apr 4', desc: 'HeyGen Pro Subscription', amount: -59, category: 'Software', type: 'expense', client: 'DigitalDNA' },
  { id: 'tr4', date: 'Apr 4', desc: 'Magnolia Concrete — Monthly', amount: 1453, category: 'Revenue', type: 'income', client: 'Magnolia Concrete' },
  { id: 'tr5', date: 'Apr 3', desc: 'Eleven Labs Creator Plan', amount: -22, category: 'Software', type: 'expense', client: 'DigitalDNA' },
  { id: 'tr6', date: 'Apr 3', desc: 'Apollo.io Pro', amount: -99, category: 'Software', type: 'expense', client: 'DigitalDNA' },
  { id: 'tr7', date: 'Apr 2', desc: 'Tidewater Lawn Care — Monthly', amount: 2347, category: 'Revenue', type: 'income', client: 'Tidewater Lawn Care' },
];
...
export default function Finances() {
  ...
  const filteredTx = mockTransactions.filter(t => {
    ...
    if (selectedClient.is_internal) return t.client === 'DigitalDNA';
    return t.client === selectedClient.business_name;
  });

  const totalRevenue = filteredTx.filter(t => t.type === 'income').reduce((a, t) => a + t.amount, 0);
  const totalExpenses = Math.abs(filteredTx.filter(t => t.type === 'expense').reduce((a, t) => a + t.amount, 0));
          ...
          <>
            ...
            <div className="rounded-lg border border-border bg-card overflow-hidden">
              <div className="px-4 py-3 border-b border-border text-sm font-semibold">
                {selectedClient ? `Transactions — ${selectedClient.business_name}` : 'Recent Transactions'}
              </div>
              {filteredTx.map((t, i) => (
                <div key={t.id} className={cn('grid px-4 py-2.5 items-center text-[13px]', showClientCol ? 'grid-cols-[80px_2fr_1fr_1fr_1fr_80px]' : 'grid-cols-[80px_2fr_1fr_1fr_80px]', i % 2 === 0 ? 'bg-card' : 'bg-background')}>
                  <span className="text-xs text-muted-foreground">{t.date}</span>
                  <span>{t.desc}</span>
                  <span className={cn('font-semibold', t.type === 'income' ? 'text-success' : 'text-destructive')}>
                    {t.amount > 0 ? '+' : ''}${Math.abs(t.amount).toLocaleString()}
                  </span>
                  {showClientCol && <span className="text-muted-foreground">{t.client}</span>}
