...
export function WelcomeTab() {
  return (
    <div className="p-10 max-w-4xl mx-auto">
      <div className="mb-10">
        <div className="text-xs uppercase tracking-[0.3em] text-[#0BA8B0] dark:text-cyan-300/80 mb-3">
          DigitalDNA Command
        </div>
        ...
        <p className="text-slate-600 dark:text-slate-300/80 text-base leading-relaxed max-w-2xl">
          {MISSION}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
        ...
        <StatCard label="Active views" value={9} />
      </div>

      <div className="grid md:grid-cols-2 gap-4">
        <InfoCard
          title="How to use this console"
          body="Use the tabs on the left to move between views. Agents shows live status. Flows lights up when agents run pipelines. Skill Trees maps what every agent can do. Crons is the weekly schedule. Team is the org chart. Console runs status pings + smoke tests."
        />
        <InfoCard
...
function StatCard({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02] p-4 backdrop-blur-sm shadow-sm dark:shadow-none">
...
function InfoCard({ title, body }: { title: string; body: string }) {
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02] p-5 shadow-sm dark:shadow-none">
